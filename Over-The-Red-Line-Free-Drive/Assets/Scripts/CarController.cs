...
public class CarController : MonoBehaviour
{
    ...
    public float springStrength = 250000f; // Cranked up for a very fast push-back
    public float springDamper = 6000f;     // Lowered significantly to remove "laziness"
    public float antiRollForce = 40000f;   // Stronger resistance to staying tilted
    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            foreach (var hit in hits)
            {
                ...
                // Spring force calculation with high strength for immediate reset
                ...
                if (Input.GetKey(KeyCode.Space) || (Mathf.Abs(v) < 0.01f && speed > 0.1f))
                    rb.AddForceAtPosition(-rb.velocity.normalized * Mathf.Min(brakeForce, speed * rb.mass) * 0.25f, wheels[i].position);
        ...
        // --- ACTIVE LEVELING (The Snap-Back Logic) ---
        // If we aren't turning and the car is tilted, we help the physics engine level out.
        if (Mathf.Abs(h) < 0.1f && groundedCount > 0)
        {
            float rollAngle = transform.localEulerAngles.z;
            if (rollAngle > 180) rollAngle -= 360;
            // Apply a corrective torque to snap the car flat
        ...
        if (groundedCount > 0)
        {
            ...
            if (speed > 1f)
            {
                ...
                rb.velocity = Vector3.Lerp(rb.velocity, new Vector3(velocityTarget.x, rb.velocity.y, velocityTarget.z), velocityAlignStrength * Time.fixedDeltaTime);
            }
        }
